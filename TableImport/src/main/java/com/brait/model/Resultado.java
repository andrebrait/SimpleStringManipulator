...
public class Resultado implements Serializable {
	...
	public Resultado(ResultadoPk id, BigDecimal mean1, BigDecimal mean2, String sheet) {
		super();
		this.id = id;
		this.mean1 = mean1;
		this.mean2 = mean2;
		this.sheet = sheet;
		updateMudancaFoldChange();
	}

	private void updateMudancaFoldChange() {
		foldChange = null;
		log2foldChange = null;
		if (mean1.compareTo(BigDecimal.ZERO) == 0 && mean2.compareTo(BigDecimal.ZERO) == 0) {
			...
			if (log2foldChange.compareTo(BigDecimal.ZERO) > 0) {
				mudanca = "UP";
			} else if (log2foldChange.compareTo(BigDecimal.ZERO) < 0) {
				mudanca = "DOWN";
			} else {
				mudanca = "SAME";
			}
